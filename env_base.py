...
class BaseEnvironment(gym.Env):
    ...
    def __init__(
        ...
        frame_width=1280,  # set to original size as default
        frame_height=720,
        render_mode=None,
    ):

        # how many frames to consider for each observation (set to 1 for a purely markovian environment)
        self.T = time_horizon
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "patch_bounding_boxes": spaces.Box(
                    low=np.zeros((self.T, self._num_patches, 4)),
                    high=np.tile(
                        np.array(
                            [
                                self.frame_width,
    ...
    def _get_observation(self, frame_index, time_horizon=None):
        ...
        for frame_idx in range(start_frame_index, end_frame_index):
            current_frame = {
                ...
                "patch_bounding_boxes": self.patch_bounding_boxes_all_frames[frame_idx],
    ...
    def step(self, action):
        ...
        # the reward is calculated only based on the immediately following frame, no matter the context!
...
if __name__ == "__main__":
    ...
    env = BaseEnvironment(
        1,  # only consider one frame for prediction => markovian
        patch_bounding_boxes,
        patch_centres,
        speaker_info,
        foa_centres,
